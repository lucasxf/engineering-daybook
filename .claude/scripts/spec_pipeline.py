...
stats = re.sub(r'(total_specs = )\d+', r'\g<1>' + str(total), stats)
for key, val in counts.items():
    stats = re.sub(rf'(\[spec_pipeline\][^\[]*\b{re.escape(key)} = )\d+', r'\g<1>' + str(val), stats)
...
if counts['approved'] == 0 and counts['implemented'] > 0:
    print(f'  Warning: Review gate bypass: {counts["implemented"]} implemented specs, 0 approved -- /review-spec is not being used')
