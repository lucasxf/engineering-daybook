...
def main():
    ...
    excl = ['__tests__/', '.test.ts', '.test.tsx', '.spec.ts', '.spec.tsx']
    web_prod = count_lines(['web/src/**/*.ts', 'web/src/**/*.tsx'], excl)
    ...
    mob_prod = count_lines(['mobile/src/**/*.ts', 'mobile/src/**/*.tsx'], excl)
    ...
    mob_e2e = count_lines(['mobile/e2e/**/*.yaml'])

    prod_total = be_prod + web_prod + mob_prod
    test_total = be_test + web_test + e2e_test + mob_test + mob_e2e
    ...
    print(f'Test LOC:       {test_total:,}  (backend={be_test:,}, web={web_test:,}, e2e={e2e_test:,}, mobile={mob_test:,}, mob_e2e={mob_e2e:,})')
