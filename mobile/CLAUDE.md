...
- **`expo prebuild --clean` re-adds `signingConfig signingConfigs.debug` to the release buildType:** Every clean prebuild resets the release signing config to the debug keystore. This makes release builds signed with the wrong key and incompatible with Play Store. The permanent fix is `mobile/plugins/withReleaseSigning.js`, which removes the spurious debug signingConfig. Two-path signing: **local builds** (`./gradlew bundleRelease`) use `signingConfigs.release` populated via env vars (ANDROID_KEYSTORE_PATH, ANDROID_KEYSTORE_PASSWORD, ANDROID_KEY_ALIAS, ANDROID_KEY_PASSWORD); **EAS cloud builds** have EAS inject signing automatically — no signingConfig line should appear in the release buildType for EAS. (Added 2026-03-19; two-path note added 2026-03-19)
