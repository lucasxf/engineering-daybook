...
EAS Android build unblocked — `androidx.core` pin (2026-04-24, PR #272): a transitive dependency resolved `androidx.core:core` and `androidx.core:core-ktx` to `1.17.0`, which requires compileSdk 36 + AGP 8.9.1. Expo SDK 53 targets compileSdk 35 + AGP 8.8.2, causing `:app:checkReleaseAarMetadata` to fail on EAS cloud. Fix: `mobile/plugins/withCorePin.js` force-pins both artifacts to `1.15.0` via `resolutionStrategy` in `app/build.gradle`. Because `android/` is committed and EAS uses it as-is (no prebuild), the block had to be materialized directly into `android/app/build.gradle` (commit `4163dda`) — registering the plugin in `app.json` alone was not sufficient. **Verified green** on EAS production build 2026-04-24. Remove this pin when upgrading to Expo SDK 54+. iOS unaffected.
