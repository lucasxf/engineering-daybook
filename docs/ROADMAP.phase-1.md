...
EAS Android build unblocked — `androidx.core` pin (2026-04-24, `fix/android-core-pin`): a transitive dependency resolved `androidx.core:core` and `androidx.core:core-ktx` to `1.17.0`, which requires compileSdk 36 + AGP 8.9.1. Expo SDK 53 targets compileSdk 35 + AGP 8.8.2, causing `:app:checkReleaseAarMetadata` to fail on EAS cloud. Fix: `mobile/plugins/withCorePin.js` force-pins both artifacts to `1.15.0` via `resolutionStrategy` in `app/build.gradle`. Remove this pin when upgrading to Expo SDK 54+. iOS unaffected.
