...
- [ ] FR3 (Must Have): Remove the exact-version pin on `expo-image-picker` (`55.0.4`) from `mobile/package.json`; upgrade to the latest compatible version (Expo SDK 53 range, i.e. `~55.x.x`).
...
**Technologies:**
- Expo SDK 53 (`~53.0.0`)
- expo-image-picker (currently pinned `55.0.4`; target `~55.x.x` post-upgrade)
...
**THEN** `package.json` has a version range (e.g. `"~55.0.x"` or `">=55.0.5"`) that is NOT the exact pin `55.0.4`  
**AND** the installed version in `node_modules` is `>= 55.0.5`
...
**Regenerated (not committed, or committed if bare workflow includes `ios/`):**
- `ios/` — `expo prebuild --clean` output; commit only if `ios/` is tracked in this repo
...
- **Files:** `mobile/package.json`, `mobile/package-lock.json`, `mobile/eas.json`
...
- **Steps:**
  ...
  3. Edit `mobile/package.json`: replace `"expo-image-picker": "55.0.4"` with latest SDK 53 compatible range (verify on npmjs.com)
  4. Run `npm install --legacy-peer-deps` in `mobile/` to regenerate lockfile
  5. Run `expo prebuild --clean` to regenerate native iOS project
