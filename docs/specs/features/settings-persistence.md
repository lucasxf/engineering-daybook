...
- [ ] **FR4:** A first-time user with no saved preferences receives theme `system` (follows OS setting) and locale derived from the device locale (EN if device locale is not PT-BR, otherwise PT-BR). `mobile`
