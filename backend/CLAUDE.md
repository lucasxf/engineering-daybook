...
- **Mockito `UnnecessaryStubbingException` — remove stubs that are unreachable on the code path under test:** Mockito strict mode (default with `@ExtendWith(MockitoExtension.class)`) flags any stub that is set up but never invoked as a test error. Two common triggers: (1) The service method throws `IllegalArgumentException` (or similar) before reaching the repository, so downstream stubs are never exercised. (2) The test exercises a branching path that simply does not call a repository method — e.g., a non-owner profile view that skips `countPoksByTagForUser`. In both cases the fix is the same: remove stubs for dependencies that are bypassed by the code path under test — only stub what will actually be called. When adding a new test scenario that exercises a different branch, audit existing stubs against the actual call graph for that branch. (Added 2026-03-07; extended 2026-04-03)
...
- **`@JsonInclude(NON_NULL)` omits null fields entirely — integration tests must assert `doesNotExist()`, not a value:** When a DTO field is annotated with `@JsonInclude(JsonInclude.Include.NON_NULL)`, a null value produces no JSON key at all — not `"field": null`. An integration test that asserts `jsonPath("$.field").value("x")` on such a field will fail with "No value at JSON path" (not "value mismatch") when the field is null. Fix the assertion to `jsonPath("$.field").doesNotExist()`. When making a previously non-null DB column nullable, always audit every integration test that asserts that field's value — they all need this correction. (Added 2026-04-03)

- **Entity `@Column(length=N)` must match the Flyway migration's column size:** In integration tests (`@ActiveProfiles("test")`), Flyway is disabled and Hibernate uses `ddl-auto: create-drop` — creating columns based on the entity annotation, not the migration SQL. If a migration widens a column (e.g. `ALTER TABLE users ALTER COLUMN handle TYPE VARCHAR(64)`) but the entity still has `@Column(length=30)`, Hibernate creates `varchar(30)` in the test DB, causing `DataIntegrityViolationException: value too long` at test runtime. Always update the entity's `@Column(length=N)` in the same commit as the migration. (Added 2026-04-18)
