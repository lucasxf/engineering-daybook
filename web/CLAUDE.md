...
- **`vi.useFakeTimers()` at module level bleeds into other test files — always add `afterAll(() => vi.useRealTimers())`:** When a test file calls `vi.useFakeTimers()` at the top level (outside a describe/beforeEach), fake timers persist across all test files that run in the same Vitest worker after it. Symptoms: slow `userEvent.type()` calls hang or timeout in unrelated test files because `setTimeout`/`setInterval` are still mocked. Fix: add `afterAll(() => { vi.useRealTimers(); })` to any test file that calls `vi.useFakeTimers()` at module level. Found in `useDebounce.test.ts` causing `PokForm.test.tsx` to fail intermittently in the full suite. (Added 2026-04-18)

- **Server layout components must use `getTranslations` (async), not `useTranslations` (client hook):** `useTranslations` from `next-intl` is a React hook — it cannot be called in async server components (layouts, pages that are not `'use client'`). Use `getTranslations(locale)` instead, which is the async server-side equivalent. Pattern for locale layouts: `const t = await getTranslations({ locale, namespace: 'MyNS' });`. Mixing these in the same file is a TypeScript error. (Added 2026-04-18)

- **Public SSG pages that need `useSearchParams` or other client hooks require `'use client'` + `<Suspense>` at the call site:** Next.js App Router cannot statically generate pages that call `useSearchParams()` unless the component is wrapped in a `<Suspense>` boundary. For public pages (e.g. `/en/support`) that are purely static but rendered in a locale layout, add `'use client'` only to the leaf component that calls the hook, and wrap the page's default export in `<Suspense fallback={null}>`. Placing `<Suspense>` in the layout rather than the page avoids the "missing Suspense boundary" build error without forcing the entire layout to be a client component. (Added 2026-04-18)
