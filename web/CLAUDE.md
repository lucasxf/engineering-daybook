...
- **`vi.hoisted()` for mocks that must be available before module evaluation:** When a mock factory references a variable that would otherwise be in TDZ (temporal dead zone) at the time `vi.mock(...)` is hoisted, wrap the shared value in `vi.hoisted(() => ...)` and reference the returned object in both the mock factory and the test body. Example: `const mockPush = vi.hoisted(() => vi.fn()); vi.mock('next/navigation', () => ({ useRouter: () => ({ push: mockPush }) }));`
