 ...
 * Navigate to /en/demos or /pt-BR/demos to access.
 *
 * ⚠️ DELETE THIS ENTIRE demos FOLDER BEFORE OPENING A PR TO PRODUCTION
 */
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-8 rounded-lg border border-warning/50 bg-warning/10 p-4">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {isPt
            ? 'Estas páginas são apenas para validação durante o desenvolvimento. Exclua a pasta demos inteira antes de abrir um PR para produção.'
            : 'These pages are for validation during development only. Delete the entire demos folder before opening a PR to production.'}
        </p>
      </div>
      ...
      <div className="space-y-4">
        {DEMOS.map((demo) => (
          <Link
            key={demo.slug}
            href={`/${params.locale}/demos/${demo.slug}`}
            className="block"
          >
      ...
      <div className="mt-8 text-center text-sm text-muted-foreground">
        {isPt ? (
          <>
            Testar em inglês:{' '}
            <Link href="/en/demos" className="text-primary hover:underline">
              /en/demos
            </Link>
          </>
        ) : (
          <>
            Test in Portuguese:{' '}
            <Link href="/pt-BR/demos" className="text-primary hover:underline">
              /pt-BR/demos
            </Link>
          </>
        )}
